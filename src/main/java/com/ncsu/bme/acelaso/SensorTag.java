...
public class SensorTag {
    ...
    public static float extractObjectTempValues (BluetoothGattCharacteristic c) {
        byte[]receive_bytes = c.getValue();
        float objectTemp = ByteBuffer.wrap(receive_bytes).order(ByteOrder.LITTLE_ENDIAN).getFloat();
        return objectTemp;
    }

    public static float extractDieTempValues (BluetoothGattCharacteristic c) {
        byte[]receive_bytes = c.getValue();
        float dieTemp = ByteBuffer.wrap(receive_bytes).order(ByteOrder.LITTLE_ENDIAN).getFloat();
        return dieTemp;
    }

    public static float extractGSRValues (BluetoothGattCharacteristic c) {
        byte[]receive_bytes = c.getValue();
